...
{
  /* MacOS X displays : characters in filenames as /, and thus allows
     (what looks like) slashes in filenames. */
  xadINT32 i;
  for(i = 0; i < size; ++i)
    if(file[i] == '/')
      file[i] = ':';
...
static xadINT32 SITmakecomment(xadSTRPTR txt, xadUINT16 creatorflags, xadSTRPTR comment, xadUINT32 csize, xadSTRPTR dest)
{
  ...
  if(res && dest)
  {
    for(i = 0; i < 4; ++i)
      ...
      *(dest++) = txt[i] ? txt[i] : '?';
#else
      *(dest++) = txt[i] >= 0x20 && txt[i] <= 0x7E ? txt[i] : '?';
#endif
    *(dest++) = '/';
    for(; i < 8; ++i)
#ifdef NO_FILENAME_MANGLING
...
XADGETINFO(SIT5)
{
  ...
  xadSize offset,diroffs;
  xadSTRPTR buffer;
  struct xadFileInfo *fi, *dir = 0, *lfi = 0;

  if((buffer = (xadSTRPTR) xadAllocVec(XADM 2048, XADMEMF_PUBLIC)))
  {
    if(!(err =  xadHookAccess(XADM XADAC_READ, 92, buffer, ai)))
    {
          ...
          if(!(err = xadHookAccess(XADM XADAC_READ, 48, buffer, ai)))
          {
            ...
            else
            {
              ...
              else if(!(err =  xadHookAccess(XADM XADAC_READ, i+j-48, buffer+48, ai)))
              {
                ...
                {
                  diroffs = EndGetM32(buffer+SIT5FH_DIRECTORY);
                  if(diroffs)
                    for(dir = ai->xai_FileInfo; dir && ((!(dir->xfi_Flags&XADFIF_DIRECTORY)) || SITDPI(dir)->Offset != diroffs); dir = dir->xfi_Next)
                      ;
                  else
                    dir = NULL;
                  ...
                  if(buffer[SIT5FH_FLAGS] & SIT5FLAGS_DIRECTORY)
                  {
                    ...
                    {
                      if((fi->xfi_FileName = MACname(xadMasterBase, dir, buffer+SIT5FH_FILENAME, nsize, 0)))
                      {
                  ...
                  else
                  {
                    ...
                    if(buffer[i+1]&0x01) /* rsrc tree */
                    {
                      ...
                      {
                        if((fi->xfi_FileName = MACname(xadMasterBase, dir, buffer+SIT5FH_FILENAME+buffer[SIT5FH_PASSWORDSIZE], nsize, 1)))
                        {
                          /* if comment field is zero, nothing is done! */
  ...
  if(SITPI(fi)->Method != 0 && SITPI(fi)->Method != 13 && SITPI(fi)->Method != 15)
  {
    ...
    SITPI(fi)->Method);
                    ...
                    if(!err && (EndGetM32(buffer+SIT5FH_DATASIZE) || !(buffer[i+1]&0x01)))
                    {
                      if((fi = xadAllocObject(XADM XADOBJ_FILEINFO, XAD_OBJPRIVINFOSIZE,
                      sizeof(struct SITPrivate), csize2 ? XAD_OBJCOMMENTSIZE : TAG_IGNORE,
                      csize2, TAG_DONE)))
                      {
                        if((fi->xfi_FileName = MACname(xadMasterBase, dir, buffer+SIT5FH_FILENAME+buffer[SIT5FH_PASSWORDSIZE], nsize, 0)))
                        {
                          SITmakecomment(buffer+i+SIT5FH_FILETYPE, EndGetM16(buffer+i+SIT5FH_FINDERFLAGS),
                          buffer+SIT5FH_FILENAME+buffer[SIT5FH_PASSWORDSIZE]+nsize+4, csize, fi->xfi_Comment);
                          if(buffer[i+1]&0x01)

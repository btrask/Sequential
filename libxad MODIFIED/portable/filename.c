...
FUNCxadConvertName /* xadUINT32 charset, xadTAGPTR tags */
{
  ...
  while((ti = NextTagItem(&ti2)))
  {
    ...
    case XAD_ERRORCODE: errcode = (xadERROR *) ti->ti_DataP; break;
    case XAD_STRINGSIZE: if(!(strs = ti->ti_Data.S)) strs = 0xFFFFFFFF; break;
    case XAD_XADSTRING:
      if(ti->ti_DataP)
      {
        obj = ((struct xadObject *)ti->ti_DataP)-1;
    ...
    case XAD_CSTRING:
      ...
      {
        ++len;
        if((i = getlen((xadUINT8 *)ti->ti_DataP, cset, strs)) < 0)
          err = -i;
        else
          len += i;
      }
  ...
  XADMEMF_CLEAR)))
  {
    ...
    while(!err && (ti = NextTagItem(&ti2)))
    {
      ...
      case XAD_PATHSEPERATOR:
        mystr->pathsep = ti->ti_DataP ? (const xadUINT16 *) ti->ti_DataP
        : psep+2; break;
      case XAD_CHARACTERSET: cset = ti->ti_Data.S; break;
      case XAD_XADSTRING:
        if(ti->ti_DataP)
        {
          ...
          if(se->xse_UnicodeSize)
            ...
            err = makestring(mystr, se->xse_Charset, (xadSTRPTR) ti->ti_DataP,
  ...
  const xadSTRING *path, const xadSTRING *name, xadTAGPTR tags */
{
  ...
  while((ti = NextTagItem(&ti2)))
  {
    ...
    case XAD_MASKCHARACTERS:
      {
        const xadSTRING *mm = (xadSTRPTR) ti->ti_DataP;
        for(i = 0; i < 256/8; ++i)
          mask[i] = 0;
        while(mm && *mm)
        {
    ...
    case XAD_REQUIREDBUFFERSIZE: reqbufsize = (xadUINT32 *) ti->ti_DataP; break;
    }
  }
  if(nopath)
  { /* skip .., . and / from path */
...
FUNCxadGetDefaultName /* xadTAGPTR tags */
{
  ...
  while((ti = NextTagItem(&ti2)))
  {
    switch(ti->ti_Tag)
    {
    case XAD_ARCHIVEINFO: ai = (struct xadArchiveInfo *) ti->ti_DataP; break;
    case XAD_ERRORCODE: errcode = (xadERROR *) ti->ti_DataP; break;
    }
  }

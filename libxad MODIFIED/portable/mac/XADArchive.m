...
NSLog(@"LITTLE ENDIAN!!!!!");
				...
				NSLog(@"Union size address: %p; pointer address: %p; corrected address", &(archive->xai_InPos.S), &(archive->xai_InPos.P), &(archive->xai_InPosP));
				archive->xai_InPos.P[0] = NULL;
				archive->xai_InPos.P[1] = NULL;
				archive->xai_InPos.P[0] = (void *)self;
				NSLog(@"%llx SHOULD NOT be %p on rosetta", archive->xai_InPos.S, self);
				archive->xai_InPos.P[0] = NULL;
				archive->xai_InPos.P[1] = NULL;
				archive->xai_InPosP = (void *)self;
				NSLog(@"%llx SHOULD be %p", archive->xai_InPos.S, self);
				return self;
			}
		}
		[self release];
...
//		name=[NSString stringWithCString:ucsstring encoding:CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingUTF16LE)];
	...
	else*/
	{
		...
		size_t const length = strlen(cname);
		NSMutableString *mutablename=[[[NSMutableString alloc] initWithBytes:cname length:length encoding:encoding] autorelease];
...
-(NSString *)commonTopDirectory
{
	...
	unsigned slash=[firstname rangeOfString:@"/"].location;

	NSString *directory;
	if(slash!=NSNotFound) directory=[firstname substringToIndex:slash];
	...
	for(int i=1;i<numentries;i++) {
		NSString *const name = [self nameOfEntry:i];
		if(![name hasPrefix:dirprefix] && ![name isEqualToString:directory]) return nil;
	}
...
-(const char *)_encodedPassword
{
	...
	NSMutableData *encoded=[[[password dataUsingEncoding:encoding] mutableCopy] autorelease];
